...
I've learned that to use the useState you have to add it to an event handler for example `onChange` and then you can specify the value from the React.useState API (value, setValue). You can also specify a default state to apply upon mount of the component, by passing a prop in the component and feeding it into the element.
