...
I've learned that custom hooks are just functions with hooks within the function.
A custom hook can help extend the use of a hook so that multiple hooks can be run in that 1 custom hook.
It can be shared and reused for different uses, so it's worth asking react developers what custom hooks they have in their dev pockets.
A custom hook has to start with "use" for example "useMyCustomHook" according to naming conventions.
